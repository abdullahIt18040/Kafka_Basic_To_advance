...
## কিভাবে পার্টিশন সংখ্যা বাড়াবে

আগে কিছু গুরুত্বপূর্ণ বিষয় জানো

Kafka-তে তুমি চাইলে টপিকের পার্টিশন সংখ্যা বাড়াতে পারবে,
কিন্তু ⚠️ কমানো যাবে না একবার তৈরি হয়ে গেলে।

এছাড়া যদি টপিকের মধ্যে আগে থেকেই ডাটা থাকে,
তাহলে নতুন পার্টিশন যোগ করলে পুরোনো মেসেজের ক্রম (order) কিছুটা পরিবর্তন হতে পারে (বিশেষ করে key hashing এর কারণে)।
```
⚙️ ধাপে ধাপে কিভাবে পার্টিশন সংখ্যা বাড়াবে
✅ ১️⃣ প্রথমে টপিকের বর্তমান অবস্থা দেখো
.\bin\windows\kafka-topics.bat --bootstrap-server localhost:9092 --describe --topic siltest


👉 এখানে তুমি দেখতে পাবে, টপিক siltest-এর এখন কয়টা পার্টিশন আছে।

✅ ২️⃣ পার্টিশন সংখ্যা বাড়ানো

ধরো, বর্তমানে তোমার টপিকে ৩টা পার্টিশন আছে,
তুমি সেটা ৬টা করতে চাও, তাহলে নিচের কমান্ডটি চালাও 👇

.\bin\windows\kafka-topics.bat --bootstrap-server localhost:9092 --alter --topic siltest --partitions 6

✅ ৩️⃣ পরিবর্তনটি সঠিকভাবে হয়েছে কিনা যাচাই করো
.\bin\windows\kafka-topics.bat --bootstrap-server localhost:9092 --describe --topic siltest


👉 এখন দেখবে কিছুটা এরকম ফলাফল আসবে:

Topic: siltest  PartitionCount: 6  ...


মানে এখন টপিকে ৬টা পার্টিশন হয়েছে।

🧩 যদি তুমি ক্লাস্টারে (একাধিক ব্রোকারে) Kafka চালাও
```
Kafka স্বয়ংক্রিয়ভাবে নতুন পার্টিশনগুলো বিভিন্ন ব্রোকারে ভাগ করে দেয়।
তবে তুমি চাইলে ম্যানুয়ালি JSON ফাইল দিয়ে নির্দিষ্ট করতে পারো কোন ব্রোকারে কোন পার্টিশন যাবে — এটা একটু অ্যাডভান্সড লেভেলের কাজ।

⚠️ মনে রাখার বিষয়

🔸 পার্টিশন সংখ্যা বাড়ানো যায়, কমানো যায় না।

🔸 পার্টিশন বাড়ানোর পর কনজিউমারদের rebalancing করতে হয় যাতে তারা নতুন পার্টিশন চিনতে পারে।


## Kafka ক্লাস্টারে নতুন Broker যোগ করা (cluster mode)

✅ ধাপ ১️⃣: নতুন কনফিগ ফাইল তৈরি করো

Kafka-র config ফোল্ডারে গিয়ে আগের কনফিগ (যেমন server_111.properties) কপি করে নতুন নামে রাখো —
উদাহরণস্বরূপ:

server_444.properties

✅ ধাপ ২️⃣: নিচের কনফিগ পরিবর্তন করো

তোমার নতুন server_444.properties ফাইল খুলে নিচের ফিল্ডগুলো বদলাও 👇
```
node.id=444
process.roles=broker
listeners=PLAINTEXT://:9098
controller.listener.names=CONTROLLER
log.dirs=/tmp/kraft-combined-logs/kafka_log_444
controller.quorum.voters=111@localhost:9093,222@localhost:9094,333@localhost:9095
```

🟢 ব্যাখ্যা:
```
broker.id=4 → নতুন ব্রোকারের আলাদা আইডি দিতে হবে

listeners → নতুন পোর্ট বা নতুন সার্ভারের IP দিতে হবে

log.dirs → নতুন ফোল্ডার যেখানে ব্রোকার তার ডেটা রাখবে

controller.quorum.voters → তোমার বর্তমান ৩টা কন্ট্রোলারের ঠিকানা থাকবে (এইটা KRaft mode-এর জন্য বাধ্যতামূলক)
...
```
অথবা, নতুন broker তৈরি করার সময় নিচের কমান্ড দিয়ে ইনিশিয়ালাইজ করতে পারো 👇
(এইটা KRaft মোডের জন্য প্রযোজ্য)

.\bin\windows\kafka-storage.bat format --cluster-id sdlcpro-01521122140 --config .\config\server_444.properties

এখানে:

--config → নতুন ব্রোকারের কনফিগ ফাইল

--cluster-id → তোমার মূল ক্লাস্টারের আইডি



✅ ধাপ–৩: এখন নতুন broker চালু করো
.\bin\windows\kafka-server-start.bat .\config\server_4.properties


এখন নতুন broker একই ক্লাস্টারে যুক্ত হয়ে যাবে।

⚠️ গুরুত্বপূর্ণ বিষয়

প্রত্যেক broker-এর broker.id আলাদা হতে হবে

কিন্তু সব broker-এর cluster.id এক হতে হবে

meta.properties ফাইল delete করে ভুল cluster.id দিলে, broker cluster-এ connect হতে পারবে না


✅ ধাপ ৬️⃣: টপিক রি-ব্যালান্স (optional)

নতুন ব্রোকারে ডেটা ছড়িয়ে দিতে হলে পুরনো টপিকগুলোর পার্টিশন reassign করতে হবে।
এর জন্য তুমি kafka-reassign-partitions টুল ব্যবহার করবে।

উদাহরণ:

.\bin\windows\kafka-reassign-partitions.bat --bootstrap-server localhost:9092 --generate --topics-to-move-json-file topics.json --broker-list "1,2,3,4"

⚠️ গুরুত্বপূর্ণ নোট

নতুন broker-কে অবশ্যই একই cluster ID-তে যুক্ত হতে হবে

controller.quorum.voters মানে হচ্ছে শুধুমাত্র controller node-গুলোর তালিকা; নতুন broker সেগুলোর সাথে যোগাযোগ করবে
